 ...
 *
 * Description:
 * This class processes files in a specified folder, uploads them to Pinata while respecting rate limits,
 * and tracks progress and errors using PerformanceMonitor. It also supports resuming uploads by checking
 * existing CID mappings to avoid re-uploading files.
 *
 * Usage:
 *   const processor = new FileUploadProcessor(pinataConfig, { maxConcurrent: 2, minTime: 2000 });
 *   const results = await processor.process({ folderPath: './my-files', outputPath: './out/file-cids.json' });
 *
 * Note:
...
export class FileUploadProcessor extends BaseFileProcessor<UploadResult[]> {
  ...
  /**
   * Constructs a new FileUploadProcessor.
   * @param config - Pinata configuration including API keys.
