...
/**
 * Describes the minimal shape required for structured result objects.
 */
type ResultShape<T> = {
...
/**
 * Determines whether a value satisfies the {@link ResultShape} contract.
 * @param value - Value under test.
 */
function isResultShape<T>(value: unknown): value is ResultShape<T> {
  if (!isObject(value)) {
    return false;
  }

  const candidate = value as Partial<ResultShape<T>>;
  return typeof candidate.success === 'boolean';
}

/**
 * Type guard for successful operation results.
 * @param result - Candidate result to evaluate.
 */
export function isSuccess<T>(result: unknown): result is ResultShape<T> & { success: true; data: T } {
  return isResultShape<T>(result) && result.success === true;
}
 ...
 * @param result - Candidate result to evaluate.
 */
export function isFailure<T>(result: unknown): result is ResultShape<T> & { success: false; error: unknown } {
  return isResultShape<T>(result) && result.success === false;
}
